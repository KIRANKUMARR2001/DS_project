...
⚙️ Predictive Maintenance ML Pipeline
...
-Dealing with predictive maintenance means handling highly imbalanced data (equipment operates normally 97% of the time and fails 3% of the time). This project specifically addresses the Precision-Recall Tradeoff.
-We evaluated both Random Forest and XGBoost. Because the business cost of a missed machine failure (downtime) is significantly higher than the cost of a false alarm (unnecessary inspection), the final model utilizes XGBoost with hyperparameter weighting (scale_pos_weight) to optimize for Recall (80%), ensuring the maximum number of true failures are caught.
...
Tech Stack:
-Language: Python
-Machine Learning: XGBoost, Scikit-Learn, Pandas,RandomForest
